  ...
  <div class="row mb-4">
    <div class="col-12 text-center">
      <h1 class="display-4 text-primary-custom font-weight-bold">
        <i class="nav-icon fa-solid fa-mountain-sun mr-2"></i> Editar {{$lugar->nombre}}
      </h1>
    </div>
  </div>
    ...
    <div class="row mb-3">
      <div class="col-12 d-flex justify-content-end">
        <button type="submit" class="btn btn-success btn-lg shadow-sm">
          <i class="fas fa-plus-circle mr-2"></i> Guardar cambios
        </button>
      </div>
    </div>
          ...
          <div class="col-md-8">
            <div class="row">
              <div class="col-md-6 mb-3">
                <x-text-input name="nombre" label="Nombre del lugar" placeholder="Ej: Pico del Destino" :value="old('nombre', $lugar->nombre)" required />
              </div>
              <div class="col-md-6 mb-3">
                <x-text-input name="otros_nombres" label="Otros nombres / Alias" placeholder="Ej: La montaña de fuego" :value="old('otros_nombres', $lugar->otros_nombres)" />
              </div>
            </div>
            <div class="row">
              <div class="col-md-4 mb-3">
                ...
                <select class="form-control select2bs4 @error('select_tipo') is-invalid @enderror" name="select_tipo" id="select_tipo" required>
                  ...
                  <option value="{{$tipo->id}}" {{ old('select_tipo', $lugar->tipo_lugar_id) == $tipo->id ? 'selected' : '' }}>{{$tipo->nombre}}</option>
                  @endforeach
                </select>
              ...
              <div class="col-md-4 mb-3">
                ...
                <select class="form-control select2bs4 @error('nivel_peligro') is-invalid @enderror" name="nivel_peligro" id="nivel_peligro">
                  ...
                  <option value="{{ $nivel }}" {{ old('nivel_peligro', $lugar->nivel_peligro) == $nivel ? 'selected' : '' }}>{{ $nivel }}</option>
                  @endforeach
                </select>
              ...
              <div class="col-md-4 mb-3">
                ...
                <select class="form-control select2bs4 @error('tipo_peligro') is-invalid @enderror" name="tipo_peligro" id="tipo_peligro">
                  ...
                  <option value="{{ $t_pel }}" {{ old('tipo_peligro', $lugar->tipo_peligro) == $t_pel ? 'selected' : '' }}>{{ $t_pel }}</option>
                  @endforeach
                </select>
            ...
            <div class="row">
              <div class="col-12 mb-3">
                ...
                <select class="form-control select2bs4 @error('dificultad_acceso') is-invalid @enderror" name="dificultad_acceso" id="dificultad_acceso">
                  ...
                  <option value="{{ $dif }}" {{ old('dificultad_acceso', $lugar->dificultad_acceso) == $dif ? 'selected' : '' }}>{{ $dif }}</option>
                  @endforeach
                </select>
                @error('dificultad_acceso') <small class="text-danger d-block">{{ $message }}</small> @enderror
              </div>
              <div class="col-12 mb-3">
                <x-text-input name="estacionalidad" label="Estacionalidad" placeholder="Ej: Solo en invierno" :value="old('estacionalidad', $lugar->estacionalidad)" />
              </div>
              <div class="col-12">
                <label class="form-label font-weight-bold">Estado</label>
                <div class="custom-control custom-switch mt-1">
                  <input type="checkbox" class="custom-control-input @error('es_secreto') is-invalid @enderror" id="es_secreto" name="es_secreto" {{ old('es_secreto', $lugar->es_secreto) ? 'checked' : '' }}>
      ...
      <div class="card-body bg-white">
        <div class="tab-content">
          <div class="tab-pane fade show active" id="tab-desc">
            <x-textarea-input name="descripcion_breve" label="Descripción general" :value="old('descripcion_breve', $lugar->descripcion_breve)" />
            <x-textarea-input name="geografia" label="Geografía detallada" :value="old('geografia', $lugar->geografia)" />
          </div>
          <div class="tab-pane fade" id="tab-entorno">
            <div class="row">
              <div class="col-md-6"><x-textarea-input name="ecosistema" label="Ecosistema" :value="old('ecosistema', $lugar->ecosistema)" /></div>
              <div class="col-md-6"><x-textarea-input name="clima" label="Clima" :value="old('clima', $lugar->clima)" /></div>
            </div>
            <x-textarea-input name="fenomeno_unico" label="Fenómeno único" :value="old('fenomeno_unico', $lugar->fenomeno_unico)" />
          </div>
          <div class="tab-pane fade" id="tab-biologia">
            <x-textarea-input name="flora_fauna" label="Flora y Fauna" :value="old('flora_fauna', $lugar->flora_fauna)" />
          </div>
          <div class="tab-pane fade" id="tab-recursos">
            <x-textarea-input name="recursos" label="Recursos generales" :value="old('recursos', $lugar->recursos)" />
          </div>
          <div class="tab-pane fade" id="tab-lore">
            <x-textarea-input name="historia" label="Historia del lugar" class="summernote" rows="12" :value="old('historia', $lugar->historia)" />
            <div class="row">
              <div class="col-md-6"><x-textarea-input name="rumores" label="Rumores y leyendas" :value="old('rumores', $lugar->rumores)" /></div>
              <div class="col-md-6"><x-textarea-input name="otros" label="Notas adicionales" :value="old('otros', $lugar->otros)" /></div>
            </div>
          </div>
        </div>
      </div>
