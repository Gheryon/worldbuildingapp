  ...
  <div class="container-fluid">
    ...
    <div class="card card-outline card-dark shadow-sm">
      ...
      <div class="card-body bg-light">
          ...
          <div class="col-md-8">
            <div class="row">
              <div class="col-md-6 mb-3">
                <x-text-input name="nombre" label="Nombre de la construcción" placeholder="Ej: Gran Faro de Pharos" :value="old('nombre')" required />
              </div>
              <div class="col-md-3">
                <x-text-input name="altitud" label="Altura" type="number" :value="old('altitud')" />
              </div>
        ...
        <div class="row">
          <div class="col-md-6">
            <x-date-input-group name="fecha_construccion" label="Fecha de construcción" :dia="old('dia_construccion')" :mes="old('mes_construccion')" :anno="old('anno_construccion')" />
          </div>
          <div class="col-md-6">
            <x-date-input-group name="fecha_destruccion" label="Fecha de destrucción/abandono" :dia="old('dia_destruccion')" :mes="old('mes_destruccion')" :anno="old('anno_destruccion')" />
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="card-body bg-white">
        <div class="tab-content">
          ...
          <div class="tab-pane fade show active" id="tab-diseno">
            <div class="row">
              <div class="col-12 mb-3"><x-textarea-input name="descripcion_breve" label="Descripción breve" :value="old('descripcion_breve')" /></div>
              <div class="col-md-6"><x-textarea-input name="arquitectura" label="Estilo arquitectónico" :value="old('arquitectura')" /></div>
              <div class="col-md-6"><x-textarea-input name="materiales_principales" label="Materiales principales" :value="old('materiales_principales')" /></div>
              <div class="col-md-6"><x-textarea-input name="tecnica_construccion" label="Técnicas de edificación" :value="old('tecnica_construccion')" /></div>
              <div class="col-md-6"><x-textarea-input name="materiales_exoticos" label="Componentes extraños o raros" :value="old('materiales_exoticos')" /></div>
            </div>
          </div>
          {{-- Pestaña Historia --}}
          <div class="tab-pane fade" id="tab-historia">
            <x-textarea-input name="proposito" label="Propósito original de la obra" :value="old('proposito')" />
            <x-textarea-input name="importancia_social" label="Relevancia cultural e importancia social" :value="old('importancia_social')" />
            <x-textarea-input name="historia" label="Historia" class="summernote" rows="12" :value="old('historia')" />
          </div>
          {{-- Pestaña Magia --}}
          <div class="tab-pane fade" id="tab-magia">
            ...
            <x-textarea-input name="propiedades_magicas" label="Propiedades y efectos mágicos" :value="old('propiedades_magicas')" />
            <x-textarea-input name="fuente_poder_magico" label="Origen o fuente del poder" :value="old('fuente_poder_magico')" />
            <x-textarea-input name="simbolismo" label="Simbolismo y esoterismo" :value="old('simbolismo')" />
          </div>
          {{-- Pestaña Acceso --}}
          <div class="tab-pane fade" id="tab-acceso">
            <x-textarea-input name="rutas_acceso" label="Rutas de acceso conocidas" :value="old('rutas_acceso')" />
            <x-textarea-input name="aspecto" label="Notas sobre el aspecto externo actual" :value="old('aspecto')" />
            <x-textarea-input name="otros" label="Notas adicionales" :value="old('otros')" />
          </div>
        </div>
      </div>
    </div>{{-- Fin panel de pestañas --}}
    <x-reference-images-manager :imagenes="old('imagenes_referencia', [])" entityType="construcciones" :entityId="null" />
    </form>
  </div>
</main>
@endsection
